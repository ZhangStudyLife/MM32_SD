...
static void safe_mkdir(const char* path)
{
	...
	if (fr && fr != FR_EXIST) show_fr("MKDIR", fr);
}
...
void all_init(void)
{
	...
	interrupt_set_priority(TIM2_IRQn, 0);
	...
	#if (FF_FS_RPATH >= 2U)
	   {
		   ...
		   if (fr) show_fr("CHDRV", fr);
	   }
	#endif
			// 创建目录（双路径尝试确保可访问）
			safe_mkdir("1:/img");        safe_mkdir("/img");
			safe_mkdir("1:/video");      safe_mkdir("/video");
			safe_mkdir("1:/cmprs");      safe_mkdir("/cmprs");
	...
	ips200_show_string(0, 212+offset, "E5:");
		...
		// KEY1 边沿检测并保存 BMP
		{
			...
			if (curr_key1 && !prev_key1)
			{
				char path[20];
				// 生成文件名，如 "1:/img/1.bmp"
				sprintf(path, "1:/img/%d.bmp", img_index);
